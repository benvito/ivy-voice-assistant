...
from config import IMG_MACRO,PRESS,IMG,POSITION,DEFINE,WAIT,AIM,CLICK,SIZE,CENTER,X,Y,POS,MOVE,STRAIGHT,WATCH,TRIGGER,LEFT_TOP,RIGHT_TOP,LEFT_BOTTOM,RIGHT_BOTTOM,LEFT_CENTER,RIGHT_CENTER
...
class IMacro:
    ...
    def get_img_pos(self, image : str) -> (int, int, int, int):
    ...
    def img_define(self, image : str or list) -> dict:
        cur_img = None
        w, h = None, None
        if type(image) != list:
            image = [image]
